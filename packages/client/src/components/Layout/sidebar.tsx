...
import { getRecentCommunities } from "@/store/recentCommunities";

export function Sidebar() {
    const communities = useCommunities();
    const nav = useNavigate();
    const recent = getRecentCommunities();
    return (
            ...
            <div className="flex flex-col text-white/90 w-full">
                ...
                {(Array.isArray(recent) && Boolean(recent.length)) && (
                    <>
                        ...
                        <div className="flex flex-col xl:mx-4">
                            <h1 className="uppercase text-xs text-gray-300/50" style={{ letterSpacing: "0.03rem" }}>Recent</h1>
                        </div>
                ...
                {(Array.isArray(communities.data) && Boolean(communities.data.filter(x => x.role == "mod" || x.role == "owner").length)) && (
                    <>
                        ...
                        {(communities.data.filter(x => x.role == "mod" || x.role == "owner").map(community => (
                            <div
                                onClick={() => nav("/c/" + community.name)}
                                className="hover:bg-[#333]/20 transition cursor-pointer flex py-3 lg:px-4 rounded-lg gap-2 justify-between text-sm items-center">
                                <div className="flex items-center gap-2">
