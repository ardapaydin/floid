...
export function CommunityIcon({ community, ...props }: {
    ...
    if (community.icon) return <img src={import.meta.env.VITE_CDN_URL + "/icons/" + community.icon} className={cn("rounded-full w-full h-full ", props.className)} draggable={false} />
    return (
