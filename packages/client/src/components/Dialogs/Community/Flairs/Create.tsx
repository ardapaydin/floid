...
import { createFlair } from "@/utils/api/flairs";
import { useQueryClient } from "@tanstack/react-query";
import { useRef, useState } from "react";
import { useParams } from "react-router-dom";

export function CreateFlair({ children }: { children: React.ReactNode }) {
    ...
    const create = async () => {
        const r = await createFlair(name!, form.flair, form.color, form.modOnly == "true")
        if (r.status == 200) {
            setIsOpen(false);
            qc.setQueryData(["communities", name!, "flairs"], (old: Flair[]) => ([...old, r.data.flair]))
        }
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
                <DialogTitle>Create New Flair</DialogTitle>
